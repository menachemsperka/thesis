"""
experiment_06_fusion_normalized.py — Fusion with Confidence Calibration

Same as experiment 06 (fusion of regular + cascaded), but with temperature scaling
calibration applied to normalize both models' confidence scores to a common scale.

This addresses the problem that regular NER (softmax) and cascade (entity_prob * bio_prob)
confidences have different statistical distributions and are not directly comparable.

Approach:
1. During training/validation: learn optimal temperature T per model via maximum likelihood
2. At test time: apply T to normalize confidences before fusion
3. Use normalized confidences for the fusion decision rule

Expected benefit:
- Calibrated confidences match empirical accuracy rates
- Comparable confidence scales across models
- Fusion can better distinguish when one model is genuinely more confident

Output: same structure as exp06 but with calibration metrics included.
"""
...
def run() -> dict:
    ...
    exp_dir = get_experiment_output_dir("exp06_fusion_normalized")
    ...
    # ── Learn calibration temperatures ──────────────────────────────────────
    ...
    merged["regular_prob_calibrated"] = merged["regular_prob"].apply(
        lambda x: apply_temperature_scaling(x, regular_temp)
    )
    merged["cascade_prob_calibrated"] = merged["cascade_prob"].apply(
    ...
    def _choose_fused(row) -> tuple[str, str, float]:
        ...
        regular_prob = float(row["regular_prob_calibrated"])
        cascade_prob = float(row["cascade_prob_calibrated"])
        ...
        [
            {
                ...
                "regular_temperature": regular_temp,
                "cascade_temperature": cascade_temp,
                "regular_ece_before": calibration_result["regular_ece_before"],
                "regular_ece_after": calibration_result["regular_ece_after"],
                "cascade_ece_before": calibration_result["cascade_ece_before"],
                "cascade_ece_after": calibration_result["cascade_ece_after"],
            }
        ]
    )
    ...
    detailed_df = merged[
        [
            ...
            "regular_pred_label",
            "regular_prob",
            "regular_prob_calibrated",
            "cascade_pred_label",
            "cascade_prob",
            "cascade_prob_calibrated",
            "disagree",
            "selected_source",
            "selected_confidence",
            "fused_pred_label",
    ...
    metrics_file = write_result_excel(
        "exp06_fusion_normalized",
        "fusion_calibrated_results",
        metrics_df,
        detailed_df,
        extra_sheets={
    ...
    result = {
        "experiment_id": "exp06_fusion_normalized",
        "name": "Fusion with Confidence Calibration (Normalized)",
        "description": "Fusion of regular NER + cascaded pipeline with temperature scaling calibration applied to normalize both models' confidence scores to a common scale before decision making.",
        ...
        "training_parameters": {
            ...
            "fusion_rule": "if regular_pred_label != cascade_pred_label, choose label with higher calibrated confidence",
            "calibration_method": "temperature scaling via maximum likelihood",
            "regular_confidence_before": "max softmax probability from regular token classifier",
            "regular_confidence_after": f"temperature-scaled: sigmoid(logit(prob) / {regular_temp:.4f})",
            "cascade_confidence_before": "(1-entity_prob) for O else entity_prob*bio_prob",
            "cascade_confidence_after": f"temperature-scaled: sigmoid(logit(prob) / {cascade_temp:.4f})",
        },
        "source_metrics_file_cascaded": str(archived_source_metrics),
        "calibration_parameters": {
            "regular_temperature": float(regular_temp),
            "cascade_temperature": float(cascade_temp),
            "regular_ece_before": float(calibration_result["regular_ece_before"]),
            "regular_ece_after": float(calibration_result["regular_ece_after"]),
            "cascade_ece_before": float(calibration_result["cascade_ece_before"]),
            "cascade_ece_after": float(calibration_result["cascade_ece_after"]),
        },
    ...
    out_path = write_result_json("exp06_fusion_normalized", "fusion_calibrated", result)
...
if __name__ == "__main__":
    ...
    if existing_seed is not None:
        payload = run()
        print(
            f"[exp06_fusion_normalized] F1={payload['f1']:.4f} | {payload['description']}"
            if payload["f1"] is not None
            else f"[exp06_fusion_normalized] F1=N/A | {payload['description']}"
        )
    else:
        ...
        for run_idx in range(1, num_runs + 1):
            ...
            print(f"[exp06_fusion_normalized] run {run_idx}/{num_runs} seed={split_seed} F1={payload.get('f1')}")
        ...
        out = write_split_runs_excel("exp06_fusion_normalized", "split_runs", runs_df, summary_df=summary_df)
        print(f"[exp06_fusion_normalized] Saved split summary: {out}")
