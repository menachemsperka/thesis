...
def run() -> dict:
    ...
    exp_dir = get_experiment_output_dir("exp05")
    timestamp = now_timestamp()
    archived_metrics_path = exp_dir / f"cascaded_pipeline_step3_consistent_results_{timestamp}.xlsx"
    ...
    result = {
        "experiment_id": "exp05",
        "name": "AUC Cascaded Pipeline (Step3 B/I Type Consistency)",
        "description": "Experiment 4 with Step 3 B/I entity-type consistency: B followed by I must share one entity type chosen by higher probability.",
        "model": model_name,
        ...
        "training_parameters": {
            ...
            "step3_bi_type_consistency": True,
        },
        "metrics_file": str(archived_metrics_path),
        "f1": f1,
        "status": "ok",
    }
    out_path = write_result_json("exp05", "auc_cascaded_pipeline_step3_consistency", result)
...
if __name__ == "__main__":
    ...
    if existing_seed is not None:
        payload = run()
        print(
            f"[exp05] Pipeline Span F1={payload['f1']:.4f} | {payload['description']}"
            if payload["f1"] is not None
            else f"[exp05] F1=N/A | {payload['description']}"
        )
    else:
        ...
        for run_idx in range(1, num_runs + 1):
            ...
            print(f"[exp05] run {run_idx}/{num_runs} seed={split_seed} F1={payload.get('f1')}")
        ...
        out = write_split_runs_excel("exp05", "split_runs", runs_df, summary_df=summary_df)
        print(f"[exp05] Saved split summary: {out}")
