"""
experiment_06_fusion_entropy.py — Fusion using entropy-weighted decisions

Better fusion strategy: Weight each model's decision by its prediction confidence/entropy.
- High-entropy (uncertain) predictions get less weight
- Boundary decisions from confident models override uncertain ones

This addresses the key issue: simple confidence comparison doesn't account for 
certainty disparity between models.
"""
            ...
            rows.append(
                {
                    ...
                    "regular_entropy": float(entropy(probs)),  # NEW: Store entropy
                }
            )
...
def _load_cascaded_predicted_tokens(excel_path: Path) -> pd.DataFrame:
    ...
    for _, row in detailed.iterrows():
        ...
        rows.append({
            ...
            "cascade_entropy": float(entropy([combined_prob, 1 - combined_prob])),  # NEW: Entropy for cascade
        })
...
def run() -> dict:
    ...
    exp_dir = get_experiment_output_dir("exp06_fusion_entropy")
    archived_source_metrics = exp_dir / f"fusion_entropy_source_cascaded_{now_timestamp()}.xlsx"
    ...
    # ============================================================================
    # ENTROPY-WEIGHTED FUSION: Key innovation
    # ============================================================================
    def _entropy_weighted_fusion(row) -> tuple[str, str, float]:
        """
        Weight predictions by confidence/entropy.
        
        Logic:
        - Compute normalized weights from entropies: weight = 1 - (entropy / max_entropy)
        - Higher entropy → lower weight
        - Weighted confidence: conf_weighted = conf * weight
        - Use prediction with higher weighted confidence
        """
        ...
        regular_entropy = float(row["regular_entropy"])
        cascade_entropy = float(row["cascade_entropy"])

        if regular_label == cascade_label:
            # Predictions agree: use prediction (average their entropies)
            return regular_label, "agree", max(regular_prob, cascade_prob)

        # Predictions disagree: use entropy-weighted comparison
        # Normalize entropies to weights (0-1, where 1 = low entropy = high confidence)
        max_entropy = max(regular_entropy, cascade_entropy)
        if max_entropy > 0:
            weight_regular = 1 - (regular_entropy / max_entropy)
            weight_cascade = 1 - (cascade_entropy / max_entropy)
        else:
            weight_regular = weight_cascade = 0.5

        # Weighted confidence: higher weight + higher base confidence = preferred
        weighted_conf_regular = regular_prob * weight_regular
        weighted_conf_cascade = cascade_prob * weight_cascade

        if weighted_conf_regular > weighted_conf_cascade:
            return regular_label, "entropy_regular", weighted_conf_regular
        else:
            return cascade_label, "entropy_cascade", weighted_conf_cascade

    selected = merged.apply(_entropy_weighted_fusion, axis=1, result_type="expand")
    ...
    selected_regular = int((merged["selected_source"] == "entropy_regular").sum())
    selected_cascade = int((merged["selected_source"] == "entropy_cascade").sum())
    ...
    metrics_file = write_result_excel(
        "exp06_fusion_entropy",
        "fusion_entropy_results",
        metrics_df,
        detailed_df,
        extra_sheets={
    ...
    result_data = {
        "experiment_id": "exp06_fusion_entropy",
        "experiment_name": "Entropy-Weighted Fusion",
        "description": "Fusion using entropy-weighted predictions. Models with lower entropy (high confidence) are trusted more.",
        "f1": float(f1),
        "precision": float(precision),
        "recall": float(recall),
        "metrics_file": str(metrics_file),
        "agreement_stats": agreement_stats,
        "fusion_method": "entropy-weighted",
    }

    out_path = write_result_json("exp06_fusion_entropy", "fusion_entropy", result_data)
