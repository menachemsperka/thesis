...
Confidence Fusion combines the regular model and the cascaded model. If both systems predict the same label, that label is retained. If they disagree, the label from the more confident system is selected. Scalar confidences are $p_i^{reg}$ (regular) and $p_i^{cas}$ (cascade). For Exp01 regular NER, $p_i^{reg}=\max_k P(y_i=k\mid x)$ from the tag softmax. For the cascaded model, if the prediction is \texttt{O} then $p_i^{cas}=1-p_i^{entity}$; otherwise $p_i^{cas}=p_i^{entity}\cdot p_i^{bio}$, where $p_i^{entity}$ and $p_i^{bio}$ are sigmoid scores from the entity and B/I heads. For token $i$:
\[
...
SVM Fusion uses the same two scalar confidences plus margins and predicted BIO/type parts on disagreement tokens. For training tokens where exactly one of the two systems is correct, a linear SVM (\texttt{LinearSVC}, $C=1$, balanced class weights, \texttt{random\_state=42}) learns which source to trust. Numeric inputs are $p_i^{reg}$, $p_i^{cas}$, both margins, confidence difference, absolute difference, and $\max(p_i^{reg},p_i^{cas})$; categorical inputs are the BIO and entity-type components of each side's prediction (one-hot encoded). Entropy is exported for analysis but is not an SVM feature in the ready implementation. The SVM then routes each disagreement token to either the regular or cascaded prediction; if the router cannot be trained, the rule above is used as fallback.
